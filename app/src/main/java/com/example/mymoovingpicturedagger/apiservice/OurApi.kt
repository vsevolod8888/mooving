...
interface OurApiService {
    ...
    suspend fun addRoute(
        @Body newRoute: SendRouteRequest,
        @Header("Authorization") token: String
    ): Response<Unit> // во все запросы подобавлять
    ...
    suspend fun deleteOneRoute(
        @Path("recordNumber") recordNumber: Long,
        @Header("Authorization") token: String
    ): Response<Unit>//Response<List<SendRouteArchive>>

    @GET("/api/tracking/coordinates/{recordNumber}") //
    suspend fun downloadOneRouteByRecordnumber(
        @Path("recordNumber") recordNumber: Long,
        @Header("Authorization") token: String
    ): Response<SendRouteRequest>
    ...
    suspend fun upLoadImageOnServer(
        @Part imageFile: MultipartBody.Part,
        @Header("Authorization") token: String
    ): Response<WhenUploadPhotoResponse>
